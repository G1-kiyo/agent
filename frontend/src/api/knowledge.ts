...
export const knowledgeApi = {
    ...
    /**
       * AI提取文本 
       * @param {FormData} formData - 上传文件
       * @returns {Promise} - 提取结果
       */
    ai_extract_text: async (formData: FormData): Promise<any> => {
    ...
    /**
       ...
       * @returns {Promise} - 进度
       */
    progress: async (batchId: string): Promise<any> => {
    ...
    /**
       ...
       */
    save_to_knowledge_base: async (formData: FormData): Promise<any> => {
    ...
    /**
       ...
       */
    rag: async function (query: string): Promise<Response> {
        const config = {
            method: 'POST',
    ...
    */
    docs: async ({ topic, pageNum, pageSize }: { topic?: string; pageNum: number; pageSize: number }): Promise<any> => {
