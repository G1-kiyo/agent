...
import { mockExtractResult, mockKeywordTopicMap } from '../pages/KnowledgePage/components/DocumentList/data/mockData'
import { BUSINESS_SUCCESS } from '../consts'
import { knowledgeApi } from '../api'

// 文档上传 hook（知识库页面专属）
// 单一职责：管理上传 Modal 状态、进度轮询、AI 元信息提取与批量提交
// 与文档列表解耦：上传成功后通过 onUploaded 回调把新文档交还列表 hook 管理
export const useDocumentUpload = ({ onUploaded } = {}) => {
  // 上传 Modal 开关
  ...
  // 读取文本类文件内容（用于 mock AI 提取，让 TXT/MD 提取结果更真实）
  ...
  // 根据内容关键词命中 mock 话题标签
  const inferTopicsFromText = (text) => {
    const lower = text.toLowerCase()
    const hit = mockKeywordTopicMap
      .filter((item) => item.keywords.some((kw) => lower.includes(kw.toLowerCase())))
      .map((item) => item.topic)
