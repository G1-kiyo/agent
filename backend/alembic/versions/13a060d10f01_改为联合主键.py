...
Revision ID: 13a060d10f01
Revises: 2c291cd23b79
Create Date: 2026-08-23 14:04:38.338691
...
revision: str = '13a060d10f01'
down_revision: Union[str, Sequence[str], None] = '2c291cd23b79'
...
def upgrade() -> None:
    ...
    op.execute("UPDATE knowledge_tag SET tag_id=83717155547250688 WHERE knowledge_id=644318752787136512")
    op.execute("ALTER TABLE knowledge_tag DROP CONSTRAINT knowledge_tag_constraint_unique")

    op.execute("""
        ALTER TABLE knowledge_tag 
        ADD PRIMARY KEY (knowledge_id,tag_id)
    """)
...
def downgrade() -> None:
    ...
    sa.UniqueConstraint('title', name=op.f('knowledge_constraint_unique'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    ...
    sa.UniqueConstraint('tag_name', name=op.f('tag_constraint_unique'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    ...
    sa.ForeignKeyConstraint(['knowledge_id'], ['knowledge.id'], name=op.f('knowledge_tag_knowledge_id_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['tag_id'], ['tag.id'], name=op.f('knowledge_tag_tag_id_fkey'), ondelete='CASCADE'),
    sa.UniqueConstraint('tag_id', 'knowledge_id', name=op.f('knowledge_tag_constraint_unique'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
    # ### end Alembic commands ###
